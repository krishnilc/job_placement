...
@section('main')
    <section class="section-5 bg-2">
        <div class="container py-5">
            <div class="row"><div class="col"><nav aria-label="breadcrumb" class="rounded-3 p-3 mb-4"><ol class="breadcrumb mb-0"><li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li><li class="breadcrumb-item active" aria-current="page">Change Password</li></ol></nav></div></div>
            <div class="row">
...
@section('customJS')
    <script>
        $('#adminPasswordForm').submit(function (event) {
            ...
            $.ajax({
                url: "{{ route('account.updatePassword') }}", type: 'POST', dataType: 'json', data: $(this).serializeArray(),
                success: function (response) {
                    if (response.status === true) { window.location.href = "{{ route('admin.account.editProfile') }}"; return; }
                    $.each(response.errors || {}, function (field, messages) { $('#' + field + 'Error').text(messages[0]); });
                },
                error: function () { alert('Unable to change your password. Please try again.'); }
            });
        });
    </script>
@endsection
